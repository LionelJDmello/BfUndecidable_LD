...
class DiagModel where
  ...
  /-- Given a program `cond`, construct the program "if cond(input) then run_forever else halt" -/
...
sense that there isn't even a total program that maps exactly those `prog : Data` to `true` for which running `prog` with input `prog` halts.
...
theorem halting_undecidable : ∀ candidate, ∃ spoiler,
  ...
  have h_func_true : eval_total candidate spoiler = true →  ¬halts spoiler spoiler :=
    ireh_runs_of_true candidate.prog spoiler (candidate.htotal spoiler)

  have h_func_false : eval_total candidate spoiler = false →  halts spoiler spoiler :=
    ireh_halts_of_false candidate.prog spoiler (candidate.htotal spoiler)

  by_cases h : (eval_total candidate spoiler = true)
  ·
    left
    exact And.intro h (h_func_true h)

  ·
    right
    simp only [Bool.not_eq_true] at h
    exact And.intro h (h_func_false h)
...
-- The use of `by_cases` makes the proof feel `less constructive` due to LEM, is there a way to prove this without?

-- Used `simp? at h` which suggested the lemma `Bool.not_eq_true` used above to simplify `¬ prop = true` to `prop = false`.

-- Is there a (≤ pain) way to achieve goals without Bool lemma?
